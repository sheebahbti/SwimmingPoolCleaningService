...
| Hosting | Render (single service — backend + frontend + DB) | Free tier, auto-deploy from GitHub |
| Database Hosting | Render PostgreSQL | Co-located with backend, low latency |
...
- Provision **Render PostgreSQL** for production
...
- **Render PostgreSQL** provides automated daily snapshots (7-day retention)
...
- Deploy backend + frontend as a **single Render service** (Express serves both API and built React static files)
- Use **Render PostgreSQL** for production DB
- Use **Cloudflare R2** for photo storage (local disk for development)
- Store all secrets in Render environment variables — never hardcode
- Set up custom domain with HTTPS (free SSL from Render)
- Set up CI/CD pipeline with **GitHub → Render** (auto-deploy on push to `main`)
